...
export default function AllUpcoming({Bookings}){
    ...
    const renderBookingCard = ({ item }) => {
        const allUsersArray = Object.values(alluser);
        console.log(allUsersArray)
        const specificUser = allUsersArray.find(user => user._id === item.hoteluserId);
        ...
        return (
            <View style={[styles.card,{width:screenWidth*0.95}]}>
                ...
                <View style={styles.header}>
                    ...
                    <View style={styles.header}>
                        <Text style={styles.heading}>Hotel Owner Name:</Text>
                        <Text style={styles.value}>   {specificUser.name}</Text>
                    </View>
                    <TouchableOpacity style={styles.header} onPress={()=>OpenDial(item.usernumber)}>
                        <Text style={styles.heading}>Hotel Owner Number:</Text>
                        <Text style={styles.value}>   {specificUser.number}</Text>
                    </TouchableOpacity>
                    <View style={styles.header}>
